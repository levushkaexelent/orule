...
function updater:download(url)
    ...
    downloadUrlToFile(url, temp_path, function(id, status)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            ...
            -- Убираем BOM если GitHub отдал с ним
            if content:sub(1, 3) == '\xEF\xBB\xBF' then
                content = content:sub(4)
            end
            
            -- Проверяем, реально ли файл в UTF-8 (ищем кириллические UTF-8 байты)
            local is_utf8 = content:find('[\xD0-\xD1][\x80-\xBF]')
            
            if is_utf8 then
                local decode_ok, decoded = pcall(u8.decode, u8, content)
