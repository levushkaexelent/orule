                            ...
                            if k then
                                ...
                                if updateversion ~= thisScript().version then
                                    lua_thread.create(function(b)
                                        ...
                                        downloadUrlToFile(updatelink, temp_file, function(n, o, p, q)
                                            ...
                                            elseif o == d.STATUS_ENDDOWNLOADDATA then
                                                ...
                                                local temp_handle = io.open(temp_file, 'rb')
                                                if temp_handle then
                                                    local utf8_content = temp_handle:read('*a')
                                                    temp_handle:close()
                                                    ...
                                                    -- Конвертация
                                                    local cp1251_content = encoding.UTF8:decode(utf8_content)
                                                    
                                                    -- Сохраняем в CP1251
                                                    local final_handle = io.open(thisScript().path, 'wb')
                                                    if final_handle then
                                                        final_handle:write(cp1251_content)
                                                        final_handle:close()
                                                        os.remove(temp_file)
                                                        ...
                                                        sampAddChatMessage(b..'Ошибка записи файла.', m)
                                                        update = false
                                                    end
                                                else
                                                    sampAddChatMessage(b..'Ошибка чтения временного файла.', m)
                                                    update = false
                                                end
                                            end
