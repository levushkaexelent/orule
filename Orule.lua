            ...
            if file then
                ...
                if success and data and data.last and data.url then
                    if data.last ~= thisScript().version then
                        sampAddChatMessage(string.format('[ORULE]  : %s -> %s.  /scriptupd', thisScript().version, data.last), 0x45AFFF)
                        
                        sampRegisterChatCommand('scriptupd', function()
                            updater:download(data.url)
                        end)
                    else
                        print('[ORULE]  : ' .. thisScript().version)
                    end
                else
                    print('[ORULE]   JSON ')
                end
...
function updater:download(url)
    ...
    downloadUrlToFile(url, temp_path, function(id, status)
        if status == dlstatus.STATUSEX_ENDDOWNLOAD then
            ...
            if not file then 
                sampAddChatMessage('[ORULE] Критическая ошибка: файл обновления не создан.', 0xFF0000)
                return 
            end
            local content = file:read('*all')
            file:close()
            ...
            if out_file then
                ...
                if os.rename(temp_path, thisScript().path) then
                    ...
                    if orule and orule.config then 
                        orule.config.lastChangelogVersion = "0"
                        saveConfig()
                    end
                    thisScript():reload()
                else
                    sampAddChatMessage('[ORULE] Ошибка: не удалось заменить файл скрипта. Проверьте права.', 0xFF0000)
                end
            else
                sampAddChatMessage('[ORULE] Ошибка: не удалось записать исправленный файл.', 0xFF0000)
            end
        elseif status == dlstatus.STATUSEX_HTTPERROR then
            sampAddChatMessage('[ORULE] Ошибка сети при скачивании обновления (HTTP Error)!', 0xFF0000)
        end
    end)
end
...
local function executeRoleplay(commands)
    if not (isSampLoaded() and isSampAvailable()) then
        sampAddChatMessage('[ORULE]    - SAMP  ', 0xFF0000)
        return
    end
...
local function saveUserQMs(data)
    ...
    if not success then
        logError('    JSON: ' .. tostring(json_str))
...
local function loadUserQMs()
    ...
    if not success or not data then
        logError('  userQMs.json: ' .. tostring(data))
        cache.userQMs = {}
        userQMs_cache_dirty = false
        return {}
    end
...
local function saveAutoInsertCards(data)
    ...
    if not file then
        logError('     : ' .. orule.AUTOINSERT_FILE)
        sampAddChatMessage('[ORULE]     ', 0xFF0000)
        return false
    end
...
local function loadAutoInsertCards()
    ...
    local success, data = pcall(decodeJson, content)
    if not success or not data then
        logError('  autoinsert_cards.json: ' .. tostring(data))
        return {}
    end
    local luaData = {}
    for i, card in ipairs(data) do
...
local function loadInteractionQMs()
    ...
    if not file then
        local defaultData = {
            {action = "showpass", content = "/me      {player}\n/me  ", title = " ", enabled = true},
            {action = "search", content = "/me  {player}\n/do   : {search_result}", title = "", enabled = true},
            {action = "cuff", content = "/me    {player}\n/do  ", title = " ", enabled = true},
            {action = "uncuff", content = "/me    {player}\n/do  ", title = " ", enabled = true},
            {action = "arrest", content = "/arrest {id}", title = "", enabled = true},
            {action = "ticket", content = "/ticket {id} 10000  ", title = " ", enabled = true}
        }
    ...
    local success, data = pcall(decodeJson, content)
    if not success or not data then
        logError('  interactionQMs.json: ' .. tostring(data))
        cache.interactionQMs = {}
        interactionQMs_cache_dirty = false
        return {}
    end
...
local function loadInteractionKeyBinds()
    ...
    local success, data = pcall(decodeJson, content)
    if not success or not data then
        logError('  interactionKeyBinds.json: ' .. tostring(data))
        return {}
    end
...
local function isShortcutAlreadyUsed(shortcut_keys, exclude_rule_index)
    ...
    for _, vk_code in ipairs(shortcut_keys) do
        if SAMP_RESERVED_KEYS[vk_code] then
            return true, "  SAMP : " .. SAMP_RESERVED_KEYS[vk_code]
        end
    end
...
local function initStaticRules()
    orule.rulesDB = {
        {name = "  ", updateDate = "08.11.2025", key = {}, keyName = " ", holdMode = false},
        {name = " ", updateDate = "08.11.2025", key = {}, keyName = " ", holdMode = false},
        {name = " \"    \"", updateDate = "08.11.2025", key = {}, keyName = " ", holdMode = false},
        {name = " \"  -   \"", updateDate = "08.11.2025", key = {}, keyName = " ", holdMode = false},
        {name = "- ", updateDate = "08.11.2025", key = {}, keyName = " ", holdMode = false},
    ...
    bg_draw_list:AddRectFilled(RENDER_CONST.ZERO, imgui.ImVec2(sw, sh), bg_color_u32)
        ...
        if rule then
            ...
            if state.overlay.rule_index == 2 then
                ...
                local button_texts = {u8'', u8' ', u8' ', u8'', u8'  ', u8'  '}
            ...
            if state.overlay.rule_index == 3 then
                ...
                local button_texts = {u8' ', u8'', u8'', u8'', u8'', u8'', u8'', u8'', u8''}
            ...
            imgui.InputTextWithHint('##search_overlay', u8'  ...', state.overlay.search_buf, 256)
            imgui.PopItemWidth()
            ...
            if state.overlay.rule_index == 3 then
               ...
               local headers = { [1]=u8'##mvd', [2]=u8'##fsb', [3]=u8'##army', [5]=u8'##mchs', [6]=u8'##hosp', [7]=u8'##smi', [8]=u8'##gov' }
                        ...
                        for i = start_i, end_i do
                            ...
                            if orule.config.showImages then
                                if state.textures.territory[i] then
                                    ...
                                    imgui.Button(u8'...##stub_' .. i, imgui.ImVec2(image_width, image_height))
                                ...
                                imgui.Button(u8'\n##disabled_' .. i, imgui.ImVec2(image_width, image_height))
                                imgui.PopStyleColor(3)
                            end
                ...
                if imgui.CollapsingHeader(u8' ##radar_map') then
                    if orule.config.showImages then
                        if state.textures.radar then
                            ...
                            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.1, 0.1, 0.1, 0.5))
                            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.1, 0.1, 0.1, 0.5))
                            imgui.Button(u8' ...##stub_radar', imgui.ImVec2(image_width, image_height))
                            imgui.PopStyleColor(3)
                        end
                    else
                        imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1.0), u8' .  " "  .')
                    end
                end
            end
...
local function renderRulesTab()
    ...
    for i, rule in ipairs(orule.rulesDB) do
        ...
        if imgui.BeginChild('##rule_card_' .. i, imgui.ImVec2(0, current_card_height), true) then
            ...
            if rule.updateDate then
                imgui.Spacing()
                imgui.TextColored(colors.text_gray, u8': ')
                imgui.SameLine(0, 5)
            ...
            local bind_text = is_capturing_this and u8'...' or u8''
            ...
            if has_key then
                ...
                if imgui.Button(u8' ##del_'..i, imgui.ImVec2(-1, 30)) and not is_capturing_any then
                    rule.key = {}
                    rule.keyName = " "
                    saveConfig()
                end
...
local function renderRadialMenuTab()
    ...
    if imgui.BeginChild('##radial_global_settings', imgui.ImVec2(0, 122), true) then
        ...
        if imgui.ToggleButton(u8'   (  )##radial_global', enabled_checkbox) and not is_capturing_any then
    ...
    for i, btn in ipairs(radialMenu.buttons) do
        ...
        if imgui.BeginChild('##radial_button_card_' .. i, imgui.ImVec2(0, 168), true) then
            ...
            imgui.Text(u8(btn.name or (' #' .. i)))
            imgui.PopStyleColor(1)
            imgui.Spacing()

            local btn_enabled = imgui.new.bool(btn.enabled)
            if imgui.ToggleButton(u8('##btn_' .. i), btn_enabled) and not is_capturing_any then
            ...
            if imgui.Button(u8('##edit_btn_' .. i), imgui.ImVec2(-1, 30)) and not is_capturing_any then
                ...
                local name_cp1251 = btn.name or (" #" .. i)
    ...
    if imgui.BeginChild('##radial_info', imgui.ImVec2(0, 350), true) then
        ...
        imgui.PushTextWrapPos(0)
        imgui.TextColored(imgui.ImVec4(0.80, 0.80, 0.85, 1.00), u8('1.   ""   \n2.   " "   \n3.   " "   \n4.  <>     (: /me <1000>/do )\n5.    ""\n6.        '))
        imgui.PopTextWrapPos()
    end
    imgui.EndChild()
    imgui.PopStyleVar(1)
...
local function renderNewsTab()
    ...
    if imgui.BeginChild('##current_version_banner', imgui.ImVec2(0, 130), true) then
        imgui.Spacing()
        if state.fonts.title then imgui.PushFont(state.fonts.title) end
        imgui.TextColored(imgui.ImVec4(0.40, 1.00, 0.50, 1.00), u8(' : ' .. SCRIPT_VERSION))
        if state.fonts.title then imgui.PopFont() end
        
        imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 1.00), u8(',   Orule!'))
        imgui.TextColored(imgui.ImVec4(0.50, 0.50, 0.55, 1.00), u8(': Lev Exelent'))
    end
    imgui.EndChild()
    imgui.PopStyleVar(2)
    imgui.PopStyleColor(2)
    ...
    if imgui.BeginChild('##news_content', imgui.ImVec2(0, 0), false) then
        ...
        if imgui.BeginChild('##v19', imgui.ImVec2(0, 230), true) then
            imgui.TextColored(imgui.ImVec4(1.00, 0.85, 0.30, 1.00), u8' v1.9.0 -  ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'    ')
            imgui.BulletText(u8'   ( )')
        ...
        if imgui.BeginChild('##v18', imgui.ImVec2(0, 200), true) then
            imgui.TextColored(imgui.ImVec4(0.90, 0.60, 1.00, 1.00), u8' v1.8.0 -   ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'  ""    ')
        ...
        if imgui.BeginChild('##v17', imgui.ImVec2(0, 180), true) then
            imgui.TextColored(imgui.ImVec4(0.40, 0.80, 0.60, 1.00), u8' v1.7 -   ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'      ')
            imgui.BulletText(u8' + L.ALT    ')
            imgui.BulletText(u8' , , / ')
            imgui.BulletText(u8'  ,     ')
        end
        imgui.EndChild()
        ...
        if imgui.BeginChild('##v16', imgui.ImVec2(0, 260), true) then
            imgui.TextColored(imgui.ImVec4(0.40, 1.00, 0.50, 1.00), u8' v1.6 -   ')
            imgui.Separator()
            imgui.Spacing()
            imgui.TextColored(imgui.ImVec4(0.50, 0.95, 0.60, 1.00), u8'  :')
            imgui.BulletText(u8'   -   ')
            imgui.BulletText(u8'  UserQMs -    ')
            imgui.BulletText(u8'      ')
            imgui.Spacing()
            imgui.TextColored(imgui.ImVec4(0.50, 0.95, 0.60, 1.00), u8'  :')
            imgui.BulletText(u8'   imgui.IsTextureValid')
            imgui.BulletText(u8'    ')
        end
        imgui.EndChild()
        imgui.PopStyleVar(1)
        ...
        if imgui.BeginChild('##v15', imgui.ImVec2(0, 175), true) then
            imgui.TextColored(imgui.ImVec4(0.50, 0.70, 1.00, 1.00), u8' v1.5 - ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'  ""   ')
            imgui.BulletText(u8'         ')
        ...
        if imgui.BeginChild('##v14', imgui.ImVec2(0, 150), true) then
            imgui.TextColored(imgui.ImVec4(0.85, 0.50, 1.00, 1.00), u8' v1.4 -  ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8' blur-     ')
            imgui.BulletText(u8'   ')
            imgui.BulletText(u8'    ')
        end
        imgui.EndChild()
        ...
        if imgui.BeginChild('##v13', imgui.ImVec2(0, 150), true) then
            imgui.TextColored(imgui.ImVec4(1.00, 0.70, 0.40, 1.00), u8' v1.3 -  ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'   (Ctrl+X, Alt+X, Shift+X)')
            imgui.BulletText(u8' 3    ')
            imgui.BulletText(u8'      SAMP')
        end
        imgui.EndChild()
        imgui.PopStyleVar(1)
        imgui.PopStyleColor(1)
        ...
        if imgui.BeginChild('##v12', imgui.ImVec2(0, 175), true) then
            imgui.TextColored(imgui.ImVec4(0.40, 0.90, 0.90, 1.00), u8' v1.2 - ')
            imgui.Separator()
            imgui.Spacing()
            imgui.BulletText(u8'    ')
            imgui.BulletText(u8' /scriptupd   ')
            imgui.BulletText(u8'   (, , )')
            imgui.BulletText(u8'    ')
        end
        imgui.EndChild()
        ...
        if imgui.BeginChild('##v11', imgui.ImVec2(0, 260), true) then
            imgui.TextColored(imgui.ImVec4(0.70, 0.50, 0.90, 1.00), u8' v1.1 -  ')
            imgui.Separator()
            imgui.Spacing()
...
local function renderSettingsTab()
    ...
    local bind_text_global = is_capturing_global and u8'   ... (Backspace )' or (u8': '..u8(getKeyName(orule.config.globalHotkey))..u8' (  )')
    ...
    if (type(orule.config.globalHotkey) == "table" and #orule.config.globalHotkey > 0) or (type(orule.config.globalHotkey) ~= "table" and orule.config.globalHotkey > 0) then
        ...
        if imgui.Button(u8' ##reset_global', imgui.ImVec2(-20, 40)) and not is_capturing_any then
    ...
    if imgui.ToggleButton(u8'   ##auto_update', auto_update_checkbox) and not is_capturing_any then
    ...
    if imgui.ToggleButton(u8'   ##show_img', show_images_checkbox) and not is_capturing_any then
    ...
    if imgui.Button(u8'  ', imgui.ImVec2(-20, 45)) and not is_capturing_any then
...
local function renderInteractionEditor()
    ...
    if imgui.Begin(u8'  ##interaction_editor', state.windows.interaction_editor, interaction_window_flags) then
        if state.interaction.edit_index > 0 then
            ...
            if item then
                imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8(': ' .. item.title))
                imgui.Spacing()
                imgui.Separator()
                imgui.Spacing()

                imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 1.00), u8' :')
                imgui.PushItemWidth(-1)
                ...
                if imgui.ToggleButton(u8('##interaction_' .. state.interaction.edit_index), enabled_checkbox) then
                ...
                if state.interaction.edit_mode == "content" then
                    imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 1.00), u8' :')
                    imgui.TextColored(imgui.ImVec4(0.60, 0.60, 0.65, 0.90), u8'  "  "  .')
                    ...
                    imgui.BeginChild("##tags_help_interaction", imgui.ImVec2(-1, 310), false, imgui.WindowFlags.NoBackground)
                        ...
                        local function copyToClipboard(text)
                            ...
                            sampAddChatMessage('[ORULE] : ' .. text, 0x00FF00)
                        end

                        imgui.Text(u8('- ')); imgui.SameLine()
                        if imgui.Button(u8('<X>'), imgui.ImVec2(50, 25)) then copyToClipboard("<X>") end
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('-  (). 1000 = 1 .'))
                        imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), u8('  : /me  <1500>/me  '))

                        imgui.Text(u8('- ')); imgui.SameLine()
                        if imgui.Button(u8('*'), imgui.ImVec2(30, 25)) then copyToClipboard("*") end
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('-   (F8)'))
                        imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), u8('  : /time<300>*'))

                        imgui.Text(u8('- ')); imgui.SameLine()
                        if imgui.Button(u8('>>X'), imgui.ImVec2(50, 25)) then copyToClipboard(">>X") end
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('-     ( )'))
                        imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), u8('  : >>/ticket'))
                        ...
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('- ID  '))
                        imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), u8('  : /cuff {id}'))
                        ...
                        imgui.SameLine()
                        imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('-   (Nick_Name)'))
                ...
                if imgui.Button(u8'  ', imgui.ImVec2(button_width, 35)) then
                    saveInteractionQMs(interactionQMs)
                ...
                local toggle_text = state.interaction.edit_mode == "content" and u8'  ' or u8' '
...
local function renderRadialEditor()
    ...
    if imgui.Begin(u8'  ##radial_editor', state.windows.radial_editor, radial_window_flags) then
        if state.radial_edit.index > 0 and state.radial_edit.index <= #radialMenu.buttons then
            ...
            if btn then
                imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8(' #' .. state.radial_edit.index))
                imgui.Spacing()
                imgui.Separator()
                imgui.Spacing()
                ...
                if imgui.ToggleButton(u8('##radial_edit'), state.radial_edit.enabled) then
                ...
                local toggle_text = state.radial_edit.mode == "content" and u8' ' or u8' '
                if imgui.Button(toggle_text, imgui.ImVec2(button_width, 35)) then
...
local function renderAutoInsertTab()
    ...
    if imgui.ToggleButton(u8' ##auto_insert', auto_insert_checkbox) then
    ...
    if imgui.Button(u8'  ', imgui.ImVec2(-1, 35)) then
        table.insert(orule.autoInsertCards, {title = " ", content = ""})
        saveAutoInsertCards(orule.autoInsertCards)
    end
    imgui.Spacing()

    imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.80, 1.00), u8(' : ') .. #orule.autoInsertCards)
    imgui.Spacing()
    imgui.Separator()
    imgui.Spacing()
    ...
    for i, card in ipairs(orule.autoInsertCards) do
        ...
        if imgui.BeginChild('##autoinsert_card_' .. i, imgui.ImVec2(0, 240), true) then
            ...
            if imgui.Button(u8'##delete_' .. i, imgui.ImVec2(80, 35)) then
                table.remove(orule.autoInsertCards, i)
                saveAutoInsertCards(orule.autoInsertCards)
                sampAddChatMessage('[ORULE]  !', 0xFF4444)
            end
            ...
            if imgui.Button(u8'##save_' .. i, imgui.ImVec2(-1, 35)) then
    ...
    if #orule.autoInsertCards == 0 then
        imgui.TextColored(imgui.ImVec4(0.60, 0.60, 0.70, 1.00), u8' .  "  "   .')
    end
end
...
local function renderInteractionTab()
    ...
    if imgui.BeginChild('##interaction_toggle', imgui.ImVec2(0, 115), true) then
        ...
        if imgui.ToggleButton(u8('  ##interaction_global'), enabled_checkbox) then
    ...
    if imgui.BeginChild('##interaction_mode', imgui.ImVec2(0, 120), true) then
        ...
        if imgui.Button(u8'##mode_keys', imgui.ImVec2(btn_width, 40)) then
        ...
        if imgui.Button(u8'##mode_menu', imgui.ImVec2(btn_width, 40)) then
    ...
    if orule.config.interactionMode == 0 then
        ...
        for i, bind in ipairs(cache.interactionKeys) do
            local action_title = action_titles[bind.action] or (bind.action .. " ()")
            ...
            if imgui.BeginChild('##key_bind_' .. i, imgui.ImVec2(0, current_card_height), true) then
                ...
                if is_capturing_this then
                    ...
                    imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.95, 0.65, 0.25, 1.00))
                    imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.85, 0.55, 0.15, 1.00))
                    if imgui.Button(u8' ...##wait_' .. i, imgui.ImVec2(-1, 30)) then
                    ...
                    imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.60, 0.55, 1.00, 0.90))
                    imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.40, 0.35, 0.90, 1.00))
                    if imgui.Button(u8('##' .. i), imgui.ImVec2(-1, 30)) and not is_capturing_any then
                        interaction_key_capture_mode = "waiting"
                        interaction_key_capture_index = i
                        sampAddChatMessage('[ORULE]        "' .. action_title .. '"', 0x00FF00)
                    end
                    imgui.PopStyleColor(3)
                end
        ...
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(0.60, 0.60, 0.70, 0.90), u8('   ( 3 )'))
        imgui.TextColored(imgui.ImVec4(0.60, 0.60, 0.70, 0.90), u8(': Ctrl + Shift + 1, Alt + 2,  3'))
    ...
    imgui.TextColored(imgui.ImVec4(0.60, 0.60, 0.70, 0.90), u8'    ,   ')
    ...
    if imgui.BeginChild('##interaction_info', imgui.ImVec2(0, 125), true) then
        imgui.Spacing()
        imgui.Indent(10)
        imgui.TextColored(imgui.ImVec4(1.00, 0.80, 0.30, 1.00), u8':')
        imgui.Spacing()
        imgui.PushTextWrapPos(0)
        imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8'    .          .')
        imgui.PopTextWrapPos()
...
local function renderAutoInsertButton()
    ...
    if imgui.Begin('##auto_insert_button', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBackground) then
        ...
        if imgui.Button(u8'', imgui.ImVec2(btn_width, btn_height)) then
    ...
    if state.autoinsert.menu_active then
        ...
        if imgui.Begin('##auto_insert_menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
            ...
            imgui.TextColored(imgui.ImVec4(0.7, 0.7, 1.0, 1.0), u8' :')
            imgui.Separator()
            imgui.Spacing()

            if #orule.autoInsertCards == 0 then
                imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.7, 1.0), u8(' .'))
                ...
                for i, card in ipairs(orule.autoInsertCards) do
                    local btn_name = (card.title and #card.title > 0) and card.title or (" #"..i)
...
local function renderInfoWindow()
    ...
    if imgui.Begin(u8'   ORULE!##info_window', state.windows.info, window_flags) then
        imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.08, 0.08, 0.12, 1.00))
        imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
        imgui.PushStyleVarFloat(imgui.StyleVar.ChildBorderSize, 1.0)
        ...
        if imgui.BeginChild('##info_content', imgui.ImVec2(0, -61), false) then
            imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'   ORULE!')
            imgui.Spacing()
            imgui.PushTextWrapPos(0)
            ...
            if imgui.BeginChild('##quick_start', imgui.ImVec2(0, 210), true) then
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 1:')
                imgui.SameLine(0, 5)
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 2:')
                imgui.SameLine(0, 5)
                imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
                imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8'     " "')
                imgui.PopTextWrapPos()
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 3:')
                imgui.SameLine(0, 5)
                imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 4:')
                imgui.SameLine(0, 5)
                imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
                imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8'     overlay  ')
                imgui.PopTextWrapPos()
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 5:')
                imgui.SameLine(0, 5)
                imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
                imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8'      ')
                imgui.PopTextWrapPos()
            ...
            if imgui.BeginChild('##radial_actions', imgui.ImVec2(0, 385), true) then
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 3 - ')
                imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 0.90), u8'      /frisk')
                imgui.Spacing()
                
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 4 - ')
                imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 0.90), u8'    /doc')
                imgui.Spacing()
                ...
                imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8' 6 - ')
                imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 0.90), u8'  ')
            ...
            imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'    1.1')
            imgui.Spacing()
            imgui.BulletText(u8'  ')
            imgui.BulletText(u8'9       ')
            imgui.BulletText(u8'   (, , , )')
            ...
            if imgui.BeginChild('##warning', imgui.ImVec2(0, 136), true) then
                imgui.Spacing()
                imgui.Indent(10)
                imgui.TextColored(imgui.ImVec4(1.00, 0.70, 0.30, 1.00), u8':')
                imgui.Spacing()
                imgui.PushTextWrapPos(imgui.GetWindowWidth() - 20)
                imgui.TextColored(imgui.ImVec4(0.85, 0.85, 0.90, 1.00), u8'     ,    "   "  ,         .')
                imgui.PopTextWrapPos()
...
local function renderPlayerMenu()
    ...
    if imgui.Begin(u8'  ##player_menu', state.windows.player_menu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
        if state.interaction.target_ped and doesCharExist(state.interaction.target_ped) then
            ...
            if result then
                imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8(" ID: " .. id))

                local player_name = sampGetPlayerNickname(id)
                if player_name then
                    imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 1.00), u8(": " .. player_name))
                end
                ...
                for i, item in ipairs(interactionQMs) do
                    if item.enabled then
                        if imgui.Button(u8(item.title), imgui.ImVec2(-1, 35)) then
                            ...
                            local player_name = sampGetPlayerNickname(id) or ""

                            
                            local content = item.content
                            content = content:gsub("{player}", player_name)
                            content = content:gsub("{id}", tostring(id))
                            content = content:gsub("{search_result}", "  ") 
                ...
                if imgui.Button(u8' (ESC)', imgui.ImVec2(-1, 30)) then
                    state.windows.player_menu[0] = false
                end
            else
                imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.5, 1.0), u8'   ID ')
            end
        else
            imgui.TextColored(imgui.ImVec4(1.0, 0.5, 0.5, 1.0), u8'  ')
        end
        ...
        if interaction_key_capture_mode == "waiting" and interaction_key_capture_index > 0 then
            ...
            elseif code ~= 0x10 and code ~= 0x11 and code ~= 0x12 then
                ...
                if #shortcut_keys >= 1 and #shortcut_keys <= 3 then
                    ...
                    sampAddChatMessage('[ORULE]  : ' .. key_text, 0x00FF00)

                    interaction_key_capture_mode = nil
                    interaction_key_capture_index = -1
                    consumeWindowMessage(true, true)
                elseif #shortcut_keys > 3 then
                    sampAddChatMessage('[ORULE]      (. 3)', 0xFF0000)
                    interaction_key_capture_mode = nil
                    interaction_key_capture_index = -1
                    consumeWindowMessage(true, true)
                end
                ...
                if #shortcut_keys >= 1 and #shortcut_keys <= 3 then
                    ...
                    if is_used then
                        ...
                        if isSampLoaded() and isSampAvailable() then
                            sampAddChatMessage(string.format('[ORULE]  "%s"   %s', shortcut_name, reason), 0xFF0000)
                        end
    ...
    if key_capture_mode and key_capture_type and (msg == wm.WM_LBUTTONDOWN or msg == wm.WM_RBUTTONDOWN or msg == wm.WM_MBUTTONDOWN or msg == wm.WM_XBUTTONDOWN) then
        ...
        if code then
            ...
            if is_used then
                ...
                if isSampLoaded() and isSampAvailable() then
                    sampAddChatMessage(string.format('[ORULE]  "%s"   %s', key_name, reason), 0xFF0000)
                end
...
local function cmd_help()
    ...
    sampAddChatMessage("Orule -   v1.6", 0x45AFFF)
    sampAddChatMessage("/" .. orule.config.command .. " -  ", 0xFFFFFF)
...
local function checkAndDownloadResources()
    ...
    sampAddChatMessage('[ORULE]  ... (' .. total .. ' )', 0xFFFFFF)
    ...
    for i, res in ipairs(resources) do
        downloadResource(res.url, res.path, res.type, function(was_downloaded)
            ...
            if processed_count % 10 == 0 or processed_count == total then
                sampAddChatMessage('[ORULE] : ' .. processed_count .. '/' .. total, 0xFFFFFF)
            end

            if processed_count == total then
                if downloaded_count > 0 then
                    sampAddChatMessage('[ORULE]   : ' .. downloaded_count, 0x00FF00)
                    if images_downloaded and orule.config.showImages then
                        reloadTextures()
                    end
                else
                    sampAddChatMessage('[ORULE]   ', 0x00FF00)
                end
            end
        end)
...
function safeMain()
    ...
    if orule.config.command and #orule.config.command > 0 then
        ...
        sampAddChatMessage("[ORULE] !  /" .. orule.config.command, 0x00FF00)
    else
        sampAddChatMessage("[ORULE] !", 0x00FF00)
    end
        ...
        if orule.config.interactionEnabled then
            ...
            elseif orule.config.interactionMode == 0 then
                ...
                if result and doesCharExist(ped) then
                    ...
                    local player_name = sampGetPlayerNickname(sampGetPlayerIdByCharHandle(ped)) or ""
