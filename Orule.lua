...
-- Объявления WinAPI функций для работы с буфером обмена
ffi.cdef[[
...
local orule = {
    ...
    autoInsertCards = {},
    online_users = {},
    selected_player_marker = nil, -- маркер выбранного игрока
    config = {
        command = "orule",
        globalHotkey = 0,
        overlayBgAlpha = 0.85,
...
-- ============================================================
-- ФИКС БАГА С ФОКУСОМ В INPUTTEXT (IMGUI)
-- ============================================================

-- Функция для сброса состояния клавиатуры и мыши
-- Решает баг с застревающим фокусом в InputText полях
...
local show_online_window = imgui.new.bool(false)
local selected_player_id = -1 -- ID выбранного игрока (-1 = никто не выбран)
...
-- Модальное окно редактирования радиального меню
...
local radial_edit_mode = "content" -- "content" или "tags"
...
-- ============================================================
-- РАБОТА С JSON ФАЙЛАМИ РАДИАЛЬНОГО МЕНЮ
-- ============================================================

-- Путь к файлу с пользовательскими отыгровками
local USER_QMS_FILE = getWorkingDirectory() .. '\\OverlayRules\\userQMs.json'

-- Путь к файлу с карточками автовставки
local AUTOINSERT_FILE = getWorkingDirectory() .. '\\OverlayRules\\autoinsert_cards.json'

-- ============================================================
-- СИСТЕМА ОТЫГРОВОК С ЗАДЕРЖКАМИ
-- ============================================================
...
local function processSpecialTags(text)
    ...
    -- Тег {time} - текущее время по МСК
    text = text:gsub("{time}", function()
        local hours = tonumber(os.date("%H", os.time() + 3 * 3600)) -- МСК = UTC + 3
...
-- Парсер команд с задержками в формате <число> и специальными командами
-- Возвращает массив команд с задержками
local function parseRoleplayCommands(content)
    ...
    -- Разбиваем по строкам и обрабатываем задержки и специальные команды
    for line in content:gmatch("[^\r\n]+") do
        line = line:gsub("^%s*(.-)%s*$", "%1") -- убираем пробелы в начале и конце

        if line ~= "" then
            -- Проверяем на задержку в формате <число>
            ...
            elseif line:match("^>>(.+)") then
                -- Ввод текста в чат и открытие чата
                ...
                -- Обычная команда чата, добавляем с текущей задержкой
                table.insert(commands, {
                    type = "chat",
                    text = line,
                    delay = current_delay
                })
                current_delay = 0 -- сбрасываем задержку после команды
            end
        end
    end
...
-- Выполнение отыгровки с поддержкой задержек и специальных команд
    ...
    lua_thread.create(function()
        for _, cmd in ipairs(commands) do
            ...
            elseif cmd.type == "screenshot" then
                -- Скриншот экрана через F8
                -- Эмулируем нажатие F8 для скриншота
                setVirtualKeyDown(119, true) -- VK_F8 = 119
                wait(50)
                setVirtualKeyDown(119, false)
            elseif cmd.type == "chat_input" and cmd.text then
                -- Ввод текста в поле чата и открытие чата
...
-- Сохранение данных радиального меню в JSON файл (CP1251 -> JSON UTF-8)
local function saveUserQMs(data)
    ...
    -- Подготовка данных для JSON (Конвертация Lua CP1251 -> UTF-8 для файла)
    local exportData = {}
    for i, item in ipairs(data) do
        exportData[i] = {
            enabled = item.enabled,
            -- Если строка есть, переводим в UTF-8, иначе пустая строка
...
-- Загрузка данных радиального меню из JSON файла (JSON UTF-8 -> CP1251 Lua)
local function loadUserQMs()
    local file = io.open(USER_QMS_FILE, 'r')
    if not file then
        -- Файл не существует, создаем дефолтный (в кодировке CP1251 для Lua)
        local defaultData = {
        ...
        -- saveUserQMs сам сконвертирует их в UTF-8 при сохранении
    ...
    -- Конвертируем полученный UTF-8 обратно в CP1251 для работы скрипта
    local luaData = {}
    for i, item in ipairs(data) do
        luaData[i] = {
            enabled = item.enabled,
            -- u8:decode превращает UTF-8 в CP1251
...
-- Сохранение карточек автовставки в JSON файл (CP1251 -> JSON UTF-8)
local function saveAutoInsertCards(data)
    ...
    -- Сохраняем в файл
    local file = io.open(AUTOINSERT_FILE, 'w')
    if not file then
        logError('Не удалось открыть файл для записи: ' .. AUTOINSERT_FILE)
...
-- Загрузка карточек автовставки из JSON файла (JSON UTF-8 -> CP1251 Lua)
local function loadAutoInsertCards()
    local file = io.open(AUTOINSERT_FILE, 'r')
    if not file then
...
-- Синхронизация данных из файла с текущими настройками радиального меню
local function syncUserQMsWithRadialMenu()
    local userData = loadUserQMs()

    -- Обновляем данные радиального меню из файла
    -- НЕ перезаписываем enabled, так как он управляется через config файл
    for i, item in ipairs(userData) do
        if radialMenu.buttons[i] then
            ...
            -- enabled теперь управляется только через config файл, а не из userQMs.json
            -- if item.enabled ~= nil then
            --     radialMenu.buttons[i].enabled = item.enabled
            -- end
        end
    end
end
...
-- ============================================================
-- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ АДАПТИВНЫХ КНОПОК
-- ============================================================

-- Функция для получения доступной ширины области
local function getContentRegionAvail()
    return imgui.GetContentRegionAvail().x
end

-- Функция для расчета ширины кнопок в ряду
...
local function executeRadialAction(index)
    ...
    -- Загружаем пользовательские отыгровки
    local userData = loadUserQMs()
    ...
    -- Парсим команды с задержками и выполняем отыгровку
...
local function saveConfig()
    ...
    for i, btn in ipairs(radialMenu.buttons) do
        -- Конвертируем название в CP1251 для сохранения в конфиг
...
local function searchInText(text, query)
    ...
    if not decode_success then
        ...
        query_cp1251 = query_text -- fallback
    end
...
local function loadConfig()
    ...
    for line in file:lines() do
        ...
        if btn_idx and radialMenu.buttons[tonumber(btn_idx)] then
            -- Конвертируем название из CP1251 в UTF-8
...
imgui.OnInitialize(function()
    ...
    if doesFileExist(font_path) then
        ...
        radial_font = io.Fonts:AddFontFromFileTTF(font_path, 24.0, nil, ranges)  -- Шрифт для радиального меню
    end
...
-- ============================================================
local function setPlayerMarker(playerId)
    -- Снимаем предыдущий маркер если есть
    if orule.selected_player_marker then
        removeBlip(orule.selected_player_marker)
        orule.selected_player_marker = nil
    end

    -- Получаем хендл игрока
    local result, player_handle = sampGetCharHandleBySampPlayerId(playerId)
    if result then
        -- Создаем маркер на игроке (красный цвет для выделения)
        orule.selected_player_marker = addBlipForChar(player_handle)
        if orule.selected_player_marker then
            changeBlipColour(orule.selected_player_marker, 0xFFFF0000) -- Красный цвет
            changeBlipScale(orule.selected_player_marker, 3) -- Увеличенный размер
            sampAddChatMessage(string.format('[ORULE] Маркер установлен на игрока ID %d', playerId), 0x00FF00)
        end
    end
end

local function removePlayerMarker()
    if orule.selected_player_marker then
        removeBlip(orule.selected_player_marker)
        orule.selected_player_marker = nil
        selected_player_id = -1
        sampAddChatMessage('[ORULE] Маркер снят', 0xFFFF00)
    end
end
...
local function updateOnlineUsers()
    ...
    if my_id then
        table.insert(orule.online_users, {
            id = my_id,
    ...
    -- В будущем здесь можно добавить логику для получения списка других пользователей скрипта
    -- через специальные сообщения в чате или другие механизмы

    -- Проверяем, существует ли ещё выбранный игрок
    if selected_player_id ~= -1 then
        local player_exists = false
        for _, user in ipairs(orule.online_users) do
            if user.id == selected_player_id then
                player_exists = true
                break
            end
        end

        -- Если выбранный игрок больше не в списке, снимаем маркер
...
imgui.OnFrame(
    ...
    function(this)
        ...
        -- Применяем более слабый blur для лучшей читаемости текста
        local bg_draw_list = imgui.GetBackgroundDrawList()
        mimgui_blur.apply(bg_draw_list, 8.0) -- Уменьшенный радиус blur

        -- Добавляем более сильное затемнение для контраста
        local bg_color_struct = imgui.ImVec4(0, 0, 0, 0.6) -- Увеличенная прозрачность для лучшей читаемости
        ...
        if rule then
            ...
            if imgui.Begin('##content_window', nil, content_flags) then
                ...
                -- Фикс бага с фокусом в InputText
                resetIO()
...
local function renderRulesTab()
    ...
    for i, rule in ipairs(orule.rulesDB) do
        ...
        if imgui.BeginChild('##rule_card_' .. i, imgui.ImVec2(-20, orule.config.ruleCardHeight), true) then
            ...
            -- Кнопка сброса - показывается только при наведении или если есть клавиша
            if imgui.IsItemHovered() or ((type(rule.key) == "table" and #rule.key > 0) or (type(rule.key) ~= "table" and rule.key > 0)) then
                ...
                local reset_button_width = 80  -- Фиксированная ширина для кнопки сброса
    ...
    for i, btn in ipairs(radialMenu.buttons) do
        ...
        if imgui.BeginChild('##radial_button_card_' .. i, imgui.ImVec2(-20, 195), true) then
            ...
            if imgui.Button(u8('Редактировать##edit_btn_' .. i), imgui.ImVec2(-20, 28)) and not is_capturing_any then
                ...
                radial_edit_mode = "content" -- Сбрасываем режим на содержимое

                -- Очищаем буферы
                ffi.fill(radial_edit_name_buffer, 64, 0)
                ffi.fill(radial_edit_content_buffer, 4096, 0)

                -- 1. Берем название (оно у нас в CP1251)
                local name_cp1251 = btn.name or ("Кнопка #" .. i)
                -- 2. Конвертируем в UTF-8 для ImGui InputText
                ...
                -- То же самое для контента: CP1251 -> UTF-8 -> Buffer
...
local function renderAutoInsertTab()
    ...
    -- Галочка включения/выключения функции автовставки
    ...
    for i, card in ipairs(orule.autoInsertCards) do
        ...
        if imgui.BeginChild('##autoinsert_card_' .. i, imgui.ImVec2(-1, 235), true) then
            ...
            -- Буфер для ввода текста (глобальный для каждой карточки)
            if not card.input_buffer then
                card.input_buffer = imgui.new.char[1024](u8(card.content or ""))
            end

            imgui.SetNextItemWidth(-1) -- Аналогично растягивает
            ...
            if imgui.Button(u8'Сохранить##save_' .. i, imgui.ImVec2(-1, 35)) then
                -- Безопасно конвертируем UTF-8 из буфера обратно в CP1251 для хранения в Lua
                local title_utf8 = ffi.string(card.title_buffer):gsub('\0', '')
                local content_utf8 = ffi.string(card.input_buffer):gsub('\0', '')

                -- Используем pcall для безопасного декодирования
...
-- ============================================================
-- АВТОВСТАВКА: КНОПКА ПОД ЧАТОМ
-- ============================================================
local function renderAutoInsertButton()
    -- Применяем основной шрифт как у всего интерфейса
    if main_font then imgui.PushFont(main_font) end
    ...
    -- == НАСТРОЙКА ПОЗИЦИИ ==
    -- X = 15 (отступ слева)
    -- Y = sh * 0.45 (примерно под чатом, чуть выше середины экрана)
    -- Можешь менять 0.45 на другое число (0.4 выше, 0.6 ниже)
    local btn_x = 15
    local btn_y = sh * 0.45 
    ...
    -- Убираем отступы (padding), чтобы кнопка была ровной и на всё окно
    imgui.PushStyleVarVec2(imgui.StyleVar.WindowPadding, imgui.ImVec2(0, 0))
    imgui.PushStyleVarFloat(imgui.StyleVar.WindowRounding, 5.0) -- Закругление
    
    -- Делаем фон самого окна прозрачным, чтобы видна была только кнопка
    imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(0, 0, 0, 0))
    imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.2, 0.4, 0.8, 0.9)) -- Цвет кнопки
    ...
    -- Флаги: без заголовка, без фона, нельзя менять размер
    ...
    -- == ОТРИСОВКА МЕНЮ (КАРТОЧЕК) ==
    -- Если меню активно, рисуем его РЯДОМ с кнопкой или ПОД ней
    if autoInsertMenuActive then
        -- Позиция меню: X такой же, Y чуть ниже кнопки
        ...
        -- Стили для меню
        imgui.PushStyleVarVec2(imgui.StyleVar.WindowPadding, imgui.ImVec2(8, 8)) -- Вернем отступы для меню
        ...
        if imgui.Begin('##auto_insert_menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
            ...
            if #orule.autoInsertCards == 0 then
                ...
                if imgui.Button(u8'Настроить', imgui.ImVec2(-1, 25)) then
                    -- Тут можно добавить открытие настроек
                    show_window[0] = true -- Открываем главное меню скрипта
                    autoInsertMenuActive = false
                end
            else
                -- Список карточек
                for i, card in ipairs(orule.autoInsertCards) do
                    -- Рисуем только если есть название
...
local function renderInfoWindow()
    local sw, sh = getScreenResolution()
    -- Адаптивный размер: максимум 75% от экрана, минимум 700x750
...
local function renderWindow()
    ...
    -- Применяем blur эффект к главному меню
    local bg_draw_list = imgui.GetBackgroundDrawList()
    mimgui_blur.apply(bg_draw_list, 12.0) -- Немного сильнее для главного меню
    ...
    -- Применяем шрифт EagleSans-Regular для заголовка окна
    ...
    if imgui.Begin(u8'Менеджер правил', show_window, imgui.WindowFlags.NoCollapse) then
        -- Снимаем шрифт заголовка сразу после Begin
        if titleFontApplied then
            imgui.PopFont()
            titleFontApplied = false
        end

        -- Применяем основной шрифт для содержимого окна
        if main_font then
    ...
    -- Проверяем триггер для открытия модального окна
    ...
    if imgui.BeginPopupModal('##radial_edit_modal', show_radial_edit_modal, imgui.WindowFlags.NoResize) then
        ...
        -- Поле для содержимого отыгровки или список тегов
        if radial_edit_mode == "content" then
            ...
            if imgui.Button(u8('<X>'), imgui.ImVec2(50, 30)) then
                -- Копируем в буфер обмена через WinAPI
                if setClipboardText then
                    setClipboardText("<X>")
                else
                    -- Fallback через WinAPI FFI
                    local user32 = ffi.load('user32')
                    local kernel32 = ffi.load('kernel32')
                    local text = "<X>"
                    local len = #text + 1
                    local hMem = kernel32.GlobalAlloc(0x0042, len) -- GMEM_MOVEABLE | GMEM_ZEROINIT
                    if hMem ~= 0 then
                        ...
                        if pMem ~= nil then
                            ...
                            user32.SetClipboardData(1, hMem) -- CF_TEXT = 1
        ...
        if imgui.Button(u8('Сохранить'), imgui.ImVec2(button_width, 35)) then
            -- Получаем данные из буферов (это UTF-8 от ImGui)
            ...
            -- Конвертируем обратно в CP1251 для хранения в памяти Lua
            ...
            elseif #title_cp1251 > 63 then
                ...
                -- Загружаем текущие данные (они придут в CP1251 благодаря новому loadUserQMs)
                local userData = loadUserQMs()
                ...
                -- Обновляем данные (в CP1251)
                userData[radial_edit_index] = {
                    title = title_cp1251,
                    content = content_cp1251,
                    enabled = radial_edit_enabled[0] -- используем состояние чекбокса из модалки
                }

                -- Обновляем кнопку в реальном времени (в памяти тоже держим CP1251)
                ...
                -- Сохраняем в файл (функция сама сконвертирует в JSON UTF-8)
        ...
        if imgui.Button(tagsButtonText, imgui.ImVec2(button_width, 35)) then
            -- Переключаем режим между содержимым и тегами
            if radial_edit_mode == "content" then
    ...
    -- Снимаем основной шрифт если он был применен
...
local function renderOnlineWindow()
    -- Обновляем список пользователей при открытии окна
    if show_online_window[0] then
        updateOnlineUsers()
    end
    ...
    if imgui.Begin(u8'Orule - Онлайн пользователи', show_online_window, imgui.WindowFlags.NoCollapse) then
        imgui.Text(u8'Пользователи скрипта онлайн:')

        if #orule.online_users == 0 then
            imgui.TextColored(imgui.ImVec4(1.0, 0.7, 0.0, 1.0), u8'Никто не найден')
        else
            imgui.Separator()
            imgui.BeginChild("##online_list", imgui.ImVec2(-1, -1), true)

            for i, user in ipairs(orule.online_users) do
                local display_text = string.format('%s[%d]', user.nickname, user.id)
                if user.is_me then
                    display_text = display_text .. u8' (Вы)'
                end

                -- Делаем ник кликабельным
                local is_selected = (selected_player_id == user.id)
                if imgui.Selectable(display_text, is_selected) then
                    if is_selected then
                        -- Снимаем маркер если игрок уже выбран
                        removePlayerMarker()
                    else
                        -- Устанавливаем маркер на выбранного игрока
                        setPlayerMarker(user.id)
                        selected_player_id = user.id
                    end
                end
                ...
                if imgui.IsItemHovered() then
                    if is_selected then
                        imgui.SetTooltip(u8'Нажмите чтобы снять маркер')
                    else
                        imgui.SetTooltip(u8'Нажмите чтобы поставить маркер')
                    end
                end

                if i < #orule.online_users then
                    imgui.Separator()
                end
            end
        ...
        -- Кнопка снятия маркера
        if selected_player_id ~= -1 then
            if imgui.Button(u8'Снять маркер', imgui.ImVec2(-1, 25)) then
                removePlayerMarker()
            end
            imgui.Spacing()
        end

        if imgui.Button(u8'Обновить список', imgui.ImVec2(-1, 25)) then
            updateOnlineUsers()
        end
    end
    imgui.End()
...
local function renderRadialMenu()
    ...
    -- Применяем шрифт для радиального меню
    ...
    -- Активируем меню при pendingActivation
    ...
    local io = imgui.GetIO()
    -- Обновляем центр каждый кадр для корректного позиционирования
    radialMenu.center = imgui.ImVec2(io.DisplaySize.x * 0.5, io.DisplaySize.y * 0.5)
    ...
    for i = 1, segments do
        ...
        -- Конвертируем UTF-8 для ImGui
        local label_u8 = u8(label)
        local textSize = imgui.CalcTextSize(label_u8)
        -- Позиционируем текст ближе к центру, чтобы не выходил за границы
    ...
    if radialMenu.action then
        ...
        radialMenu.releasePending = false -- Сбрасываем флаг после выполнения
    end
    ...
    -- Снимаем шрифт если он был применен
    if fontApplied then
        imgui.PopFont()
    end
end
...
local function shouldProcessImGui()
    return show_window[0] or show_info_window[0] or show_online_window[0] or radialMenu.active or radialMenu.pendingActivation or radialMenu.releasePending or radialMenu.isHeld
end

imgui.OnFrame(function()
    return shouldProcessImGui() or (orule.config.autoInsertEnabled and sampIsChatInputActive())
end, function()
    ...
    -- Автовставка: рисуем кнопку под чатом, если функция включена
...
function onWindowMessage(msg, wparam, lparam)
    ...
    if not key_capture_mode and radialMenu.globalEnabled then
        if msg == wm.WM_MBUTTONDOWN then
            ...
            -- consumeWindowMessage(true, true) -- УБРАНО: чтобы ImGui получал сигналы мыши
            imgui.Process = true
...
local function cmd_help()
    ...
    sampAddChatMessage("Orule - Менеджер правил v1.5", 0x45AFFF)
...
function safeMain()
    ...
    if orule.config.command and #orule.config.command > 0 then
        ...
        sampRegisterChatCommand('online', function() show_online_window[0] = not show_online_window[0] end)
    end
...
function onScriptTerminate(script, quitGame)
    if script == thisScript() then
        -- Очищаем маркер при выгрузке скрипта
        removePlayerMarker()
    end
end
