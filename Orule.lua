...
local MINIMUM_MOONLOADER_VERSION = 026
local DEFAULT_WINDOW_WIDTH = 820
local DEFAULT_WINDOW_HEIGHT = 800
local DEFAULT_RULE_CARD_HEIGHT = 183
local MAX_KEY_CODE = 511

local updater = {
    raw = 'https://raw.githubusercontent.com/levushkaexelent/orule/refs/heads/main/update.json'
}

function updater:getLastVersion()
    local requests = require('requests')
    local response = requests.get(self.raw)
    ...
    if response.status_code == 200 then
        ...
        downloadUrlToFile(script_url, thisScript().path, function (id, status, p1, p2)
            ...
            if status == dlstatus.STATUSEX_ENDDOWNLOAD then
                ...
                -- Сбрасываем версию changelog чтобы показать окно после перезагрузки
...
local orule = {
    ...
    config = {
        command = "orule",
        lastChangelogVersion = "0", -- Версия последнего просмотренного changelog'а
        globalHotkey = 0,
        overlayBgAlpha = 0.85,
        fontSize = 18.0,
        lineSpacing = 0.0,
        ...
        ruleCardHeight = DEFAULT_RULE_CARD_HEIGHT,
        firstLaunch = true,
        autoUpdateTexts = true,
        autoInsertEnabled = true,
        interactionEnabled = true,
        interactionMode = 1 -- 0 = Клавиши, 1 = Меню
    }
}

orule.CONFIG_FILE = orule.SCRIPT_DIR .. '\\config.txt'
orule.AUTOINSERT_FILE = orule.SCRIPT_DIR .. '\\autoinsert_cards.json'
...
local function cp1251_lower(str)
    if type(str) ~= "string" then return string.lower(str) end
...
local show_changelog_window = imgui.new.bool(false)
local show_player_menu = imgui.new.bool(false)
local target_ped_handle = nil
local show_interaction_editor = imgui.new.bool(false)
local editing_interaction_index = -1
local interaction_edit_mode = "content" -- "content" или "tags"

-- Бинды для режима клавиш взаимодействия
local interaction_key_binds = {} -- {action = "showpass", key = {16, 49}} -- Shift + 1
local interaction_key_capture_mode = nil -- nil, "waiting", или индекс действия
local interaction_key_capture_index = -1 -- Индекс редактируемого действия взаимодействия
local interaction_edit_name_buffer = imgui.new.char[64]()
local interaction_edit_content_buffer = imgui.new.char[4096]()
local interaction_edit_enabled = imgui.new.bool(true)
local CHANGELOG_VERSION = "1.8" -- Версия changelog'а, меняй при каждом обновлении
local info_window_shown_once = false
local main_font, title_font, radial_font = nil, nil, nil
local overlay_visible = false
local overlay_rule_index = nil
local autoInsertMenuActive = false
local interactionMenuActive = false
local interactionMode = imgui.new.int(1) -- 0 = Клавиши, 1 = Меню

local show_radial_edit_modal = imgui.new.bool(false)
local radial_edit_index = 0
local radial_edit_name_buffer = imgui.new.char[64]()
local radial_edit_content_buffer = imgui.new.char[4096]()
local radial_edit_enabled = imgui.new.bool(true)
local should_open_radial_modal = false
local radial_edit_mode = "content"
...
-- Кеш для отыгровок взаимодействия
local interactionQMs_cache = nil
...
local function loadUserQMs()
    if not userQMs_cache_dirty and userQMs_cache then
        return userQMs_cache
    end
...
local function loadInteractionQMs()
    if not interactionQMs_cache_dirty and interactionQMs_cache then
        return interactionQMs_cache
    end
    ...
    if not file then
        ...
        interactionQMs_cache = defaultData
        interactionQMs_cache_dirty = false
        return defaultData
    end
...
local function loadInteractionKeyBinds()
    ...
    if not file then
        ...
        return {
            {action = "showpass", key = {49}}, -- 1
            {action = "search", key = {50}}, -- 2
            {action = "cuff", key = {51}}, -- 3
            {action = "uncuff", key = {52}}, -- 4
            {action = "arrest", key = {53}}, -- 5
            {action = "ticket", key = {54}} -- 6
        }
    end
...
local function saveConfig()
    ...
    local hotkey = orule.config.globalHotkey
    local hotkeyStr = type(hotkey) == "table" and table.concat(hotkey, ",") or tostring(hotkey or 0)
    file:write("globalHotkey:" .. hotkeyStr .. "\n")
    file:write("windowWidth:" .. tostring(orule.config.windowWidth or 750) .. "\n")
    file:write("windowHeight:" .. tostring(orule.config.windowHeight or 700) .. "\n")
    ...
    file:write("autoInsertEnabled:" .. tostring(orule.config.autoInsertEnabled and "1" or "0") .. "\n")
    file:write("lastChangelogVersion:" .. (orule.config.lastChangelogVersion or "0") .. "\n")
    file:write("interactionEnabled:" .. tostring(orule.config.interactionEnabled and "1" or "0") .. "\n")
    file:write("interactionMode:" .. tostring(orule.config.interactionMode or 1) .. "\n")
    for i, btn in ipairs(radialMenu.buttons) do
        local name_cp1251 = btn.name or ""
        if name_cp1251 ~= "" then
            local success, converted = pcall(encoding.CP1251.encode, encoding.CP1251, name_cp1251)
            if success then
                name_cp1251 = converted
            end
        end
        file:write("radialButton_" .. i .. "_name:" .. name_cp1251 .. "\n")
...
local function scheduleConfigSave()
    ...
    config_save_timer = os.clock() * 1000
end
...
local function searchInText(text, query)
    ...
    local decode_success, query_cp1251 = pcall(encoding.UTF8.decode, encoding.UTF8, query_text)
    if not decode_success then
        logError("Ошибка декодирования UTF-8 в поиске: " .. tostring(query_cp1251))
        query_cp1251 = query_text
    end
...
local function searchInTextCached(full_text, query, rule_index, tab_index)
    local query_str = query and ffi.string(query) or ""

    if search_cache.query == query_str
       and search_cache.rule_index == rule_index
       and search_cache.tab_index == tab_index
       and search_cache.full_text == full_text then
        return search_cache.results, search_cache.lines
    end

    local results, lines = searchInText(full_text, query)

    search_cache.query = query_str
    search_cache.rule_index = rule_index
    search_cache.tab_index = tab_index
    search_cache.full_text = full_text
    search_cache.results = results
    search_cache.lines = lines
...
local function loadConfig()
    ...
    for line in file:lines() do
        ...
        if wwidth then orule.config.windowWidth = tonumber(wwidth) or 750 end

        local wheight = line:match("^windowHeight:(.+)")
        if wheight then orule.config.windowHeight = tonumber(wheight) or 700 end
        ...
        local auto_insert_enabled = line:match("^autoInsertEnabled:(.+)")
        if auto_insert_enabled then
            orule.config.autoInsertEnabled = (auto_insert_enabled == "1")
        end

        local changelog_ver = line:match("^lastChangelogVersion:(.+)")
        if changelog_ver then
            orule.config.lastChangelogVersion = changelog_ver
        end

        local last_changelog_version = line:match("^lastChangelogVersion:(.+)")
        if last_changelog_version then
            orule.config.lastChangelogVersion = last_changelog_version
        end

        local interaction_enabled = line:match("^interactionEnabled:(.+)")
        if interaction_enabled then
            orule.config.interactionEnabled = (interaction_enabled == "1")
        end

        local interaction_mode = line:match("^interactionMode:(.+)")
        if interaction_mode then
            orule.config.interactionMode = tonumber(interaction_mode) or 1
            interactionMode[0] = orule.config.interactionMode
        end

        local btn_idx, btn_name = line:match("^radialButton_(%d+)_name:(.+)")
        if btn_idx and radialMenu.buttons[tonumber(btn_idx)] then
            if btn_name and btn_name ~= "" then
                local success, converted = pcall(encoding.CP1251.decode, encoding.CP1251, btn_name)
                if success then
                    btn_name = converted
                end
            end
        ...
        if r_idx then
            local rule_index = tonumber(r_idx)
            local rule = orule.rulesDB[rule_index]
            if rule then
                if string.find(r_key, ",") then
                    ...
                    if #key_array > 0 then
                        rule.key = key_array
                        rule.keyName = getShortcutName(key_array)
                    end
                else
                    local key_val = tonumber(r_key) or 0
                    rule.key = key_val
                    rule.keyName = getKeyName(key_val)
                end
            end
        end
        ...
        if h_idx then
            local rule_index = tonumber(h_idx)
            local rule = orule.rulesDB[rule_index]
            if rule then
                rule.holdMode = (h_mode == "1")
            end
        end
    end
    file:close()
...
imgui.OnInitialize(function()
    ...
    if doesFileExist(font_path) then
        ...
        radial_font = io.Fonts:AddFontFromFileTTF(font_path, 24.0, nil, ranges)
    end
...
imgui.OnFrame(
    ...
    function(this)
        ...
        local bg_color_struct = imgui.ImVec4(0, 0, 0, 0.6)
        local bg_color_u32 = imgui.ColorConvertFloat4ToU32(bg_color_struct)
        bg_draw_list:AddRectFilled(imgui.ImVec2(0, 0), imgui.ImVec2(sw, sh), bg_color_u32)
        ...
        if rule then
            ...
            if imgui.Begin('##content_window', nil, content_flags) then
                ...
                if overlay_rule_index == 1 then
                    imgui.Spacing()
                    local button_width = imgui.GetMiddleButtonX(4)
                ...
                if overlay_rule_index == 2 then
                    imgui.Spacing()
                    local button_width = imgui.GetMiddleButtonX(3)
                ...
                imgui.PushStyleVarVec2(imgui.StyleVar.FramePadding, imgui.ImVec2(35.0, 8.0))
                ...
                if full_text and #full_text > 0 then
                    ...
                    local current_tab = 0
                    if overlay_rule_index == 1 then
                        current_tab = policeRuleActiveTab[0] or 0
                    elseif overlay_rule_index == 2 then
                        current_tab = legalBaseActiveTab[0] or 0
                    elseif overlay_rule_index == 3 then
                        current_tab = territoryActiveTab[0] or 0
                    end
                    renderSearchResults(full_text, query, overlay_rule_index, current_tab)
                end
...
local function renderRulesTab()
    ...
    for i, rule in ipairs(orule.rulesDB) do
        ...
        if imgui.BeginChild('##rule_card_' .. i, imgui.ImVec2(-20, orule.config.ruleCardHeight), true) then
            ...
            local button_width = imgui.GetMiddleButtonX(2)
            ...
            if imgui.IsItemHovered() or ((type(rule.key) == "table" and #rule.key > 0) or (type(rule.key) ~= "table" and rule.key > 0)) then
                imgui.SameLine()
                ...
                if imgui.Button(u8'?##reset_'..i, imgui.ImVec2(reset_button_width, button_height)) and not is_capturing_any then
                    rule.key = {}
                    rule.keyName = "Не назначено"
                    saveConfig()
                end
                if imgui.IsItemHovered() then
                    imgui.SetTooltip(u8'Сбросить клавишу')
                end
                imgui.PopStyleColor(3)
            end
...
local function renderRadialMenuTab()
    ...
    for i, btn in ipairs(radialMenu.buttons) do
        ...
        if imgui.BeginChild('##radial_button_card_' .. i, imgui.ImVec2(-20, 195), true) then
            imgui.Spacing()
            imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(0.95, 0.95, 1.00, 1.00))
            ...
            imgui.Spacing()
            imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8('Название: ') .. u8(btn.name))
            ...
            if imgui.Button(u8('Редактировать##edit_btn_' .. i), imgui.ImVec2(-20, 28)) and not is_capturing_any then
                radial_edit_index = i
                radial_edit_enabled[0] = btn.enabled
                radial_edit_mode = "content"
                ffi.fill(radial_edit_name_buffer, 64, 0)
                ffi.fill(radial_edit_content_buffer, 4096, 0)

                local name_cp1251 = btn.name or ("Кнопка #" .. i)
                local name_utf8 = u8(name_cp1251)
                ffi.copy(radial_edit_name_buffer, name_utf8, math.min(#name_utf8, 63))
                local userData = loadUserQMs()
                ...
                ffi.copy(radial_edit_content_buffer, content_utf8, math.min(#content_utf8, 4095))

                show_radial_edit_modal[0] = true
                should_open_radial_modal = true
                resetIO()
            end
            imgui.PopStyleColor(3)
            imgui.Spacing()
    ...
    imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 6.0)
    if imgui.BeginChild('##radial_info', imgui.ImVec2(-20, 350), true) then
        imgui.Spacing()
...
local function renderInteractionEditor()
    ...
    imgui.SetNextWindowSize(imgui.ImVec2(600, 620), imgui.Cond.Always)
    ...
    if imgui.Begin(u8'Редактор отыгровок взаимодействия', show_interaction_editor, imgui.WindowFlags.NoCollapse) then
        if editing_interaction_index > 0 then
            local interactionQMs = loadInteractionQMs()
            local item = interactionQMs[editing_interaction_index]
            ...
            if item then
                ...
                -- Название действия (всегда видно)
                imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 1.00), u8'Название действия:')
                imgui.PushItemWidth(-1)
                if imgui.InputText('##interaction_name', interaction_edit_name_buffer, 64) then
                    item.title = u8:decode(ffi.string(interaction_edit_name_buffer))
                ...
                -- Включено/выключено (всегда видно)
                local enabled_checkbox = imgui.new.bool(item.enabled)
                if imgui.Checkbox(u8'Включено', enabled_checkbox) then
                ...
                -- ЛОГИКА ПЕРЕКЛЮЧЕНИЯ (Контент / Теги)
                if interaction_edit_mode == "content" then
                    ...
                    local text_height = 280 -- Увеличили высоту, так как убрали поле "Действие"
                    ...
                    if imgui.InputTextMultiline('##interaction_content', interaction_edit_content_buffer, 4096,
                        imgui.ImVec2(-1, text_height)) then
                        item.content = u8:decode(ffi.string(interaction_edit_content_buffer))
                        saveInteractionQMs(interactionQMs)
                    end
                ...
                if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(button_width, 35)) then
                    saveInteractionQMs(interactionQMs)
                    show_interaction_editor[0] = false
                    editing_interaction_index = -1
                end

                imgui.SameLine()

                -- Кнопка переключения режима
                local toggle_text = interaction_edit_mode == "content" and u8'Теги и функции' or u8'К содержимому'
                if imgui.Button(toggle_text, imgui.ImVec2(button_width, 35)) then
                    interaction_edit_mode = (interaction_edit_mode == "content") and "tags" or "content"
                end
...
local function renderAutoInsertTab()
    ...
    if imgui.Checkbox(u8'Включить автовставку', auto_insert_checkbox) then
        orule.config.autoInsertEnabled = auto_insert_checkbox[0]
        saveConfig()
    end

    imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'Автовставка')
    imgui.Spacing()
    ...
    for i, card in ipairs(orule.autoInsertCards) do
        ...
        if imgui.BeginChild('##autoinsert_card_' .. i, imgui.ImVec2(-1, 235), true) then
            imgui.Spacing()

            imgui.TextColored(imgui.ImVec4(0.75, 0.75, 0.80, 0.90), u8'Название карточки:')

            if not card.title_buffer then
                card.title_buffer = imgui.new.char[256](u8(card.title or ""))
            end
            ...
            if not card.input_buffer then
                card.input_buffer = imgui.new.char[1024](u8(card.content or ""))
            end
...
local function renderInteractionTab()
    -- Заголовок
    imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'Взаимодействие с игроками')
    imgui.Spacing()
    imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 0.90), u8'Быстрые действия для взаимодействия с другими игроками')
    imgui.Spacing()
    imgui.Spacing()

    -- Карточка включения/выключения
    imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.11, 0.11, 0.13, 1.00))
    imgui.PushStyleColor(imgui.Col.Border, imgui.ImVec4(0.30, 0.30, 0.35, 0.50))
    imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
    imgui.PushStyleVarFloat(imgui.StyleVar.ChildBorderSize, 1.5)

    if imgui.BeginChild('##interaction_toggle', imgui.ImVec2(-20, 110), true) then
        imgui.Spacing()
        imgui.Indent(10)

        local enabled_checkbox = imgui.new.bool(orule.config.interactionEnabled)
        if imgui.Checkbox(u8'Включить модуль взаимодействия', enabled_checkbox) then
    ...
    imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'Режим активации')
    imgui.Spacing()
    ...
    if imgui.BeginChild('##interaction_mode', imgui.ImVec2(-20, 120), true) then
        imgui.Spacing()
        imgui.Indent(10)

        local total_width = imgui.GetWindowWidth() - 30
        local btn_width = (total_width - 10) / 2  -- Добавляем отступ между кнопками

        -- Убираем отступ между кнопками
        imgui.PushStyleVarVec2(imgui.StyleVar.ItemSpacing, imgui.ImVec2(0, 0))
        -- Убираем скругление у кнопок
        imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, 0)

        -- Кнопка "Клавиши"
        local keys_active = (interactionMode[0] == 0)
        if keys_active then
        ...
        -- Кнопка "Меню"
        local menu_active = (interactionMode[0] == 1)
        if menu_active then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.50, 0.45, 1.00, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.55, 0.50, 1.00, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.45, 0.40, 0.95, 1.00))
        ...
        imgui.PopStyleVar(2) -- Возвращаем ItemSpacing и FrameRounding

        -- Добавляем отступ справа после второй кнопки
        imgui.SameLine()
        imgui.Dummy(imgui.ImVec2(5, 0))

        local mode_desc = interactionMode[0] == 0
    ...
    if interactionMode[0] == 0 then
        -- Режим клавиш - настройка биндов
        imgui.TextColored(imgui.ImVec4(0.50, 0.45, 1.00, 1.00), u8'Настройка горячих клавиш')
        imgui.Spacing()
        imgui.TextColored(imgui.ImVec4(0.70, 0.70, 0.75, 0.90), u8'Наведите прицел на игрока и нажмите клавишу/комбинацию')
        imgui.Spacing()
        imgui.Spacing()

        -- Загружаем отыгровки и бинды
        local interactionQMs = loadInteractionQMs()

        -- Создаем маппинг действий для отображения
        local action_titles = {}
        ...
        -- Отображаем текущие бинды
        for i, bind in ipairs(interaction_key_binds) do
            ...
            if imgui.BeginChild('##key_bind_' .. i, imgui.ImVec2(-20, 115), true) then
                ...
                if interaction_key_capture_mode == "waiting" and interaction_key_capture_index == i then
                    if imgui.Button(u8'Нажмите клавишу...', imgui.ImVec2(100, 25)) then
                        -- Ничего не делаем, ждем клавишу
                    end
                else
                    if imgui.Button(u8('Изменить##' .. i), imgui.ImVec2(100, 25)) then
        ...
        if imgui.BeginChild('##interaction_preview', imgui.ImVec2(-20, 365), true) then
            imgui.Spacing()
            ...
            -- Пункты меню (превью) - загружаем из настроек
            local interactionQMs = loadInteractionQMs()
            ...
            for i, action_data in ipairs(enabled_actions) do
                ...
                if imgui.Button(btn_text .. '##preview_' .. i, imgui.ImVec2(-1, 32)) then
                    -- Открываем редактор для этого действия
                    show_interaction_editor[0] = true
                    editing_interaction_index = original_index
                    interaction_edit_mode = "content" -- Сброс режима
                    interaction_edit_name_buffer = imgui.new.char[64](u8(item.title))
                    interaction_edit_content_buffer = imgui.new.char[4096](u8(item.content))
                    interaction_edit_enabled[0] = item.enabled
                end
    ...
    imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.08, 0.08, 0.12, 1.00))
    imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 6.0)
    if imgui.BeginChild('##interaction_info', imgui.ImVec2(-20, 125), true) then
        imgui.Spacing()
        imgui.Indent(10)
...
local function renderAutoInsertButton()
    ...
    if imgui.Begin('##auto_insert_button', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBackground) then
        ...
        if imgui.Button(u8'Автовставка', imgui.ImVec2(btn_width, btn_height)) then
            autoInsertMenuActive = not autoInsertMenuActive
        end
        ...
        if imgui.Begin('##auto_insert_menu', nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
            ...
            if #orule.autoInsertCards == 0 then
                ...
                if imgui.Button(u8'Настроить', imgui.ImVec2(-1, 25)) then
                    show_window[0] = true
                    autoInsertMenuActive = false
                end
            else
                for i, card in ipairs(orule.autoInsertCards) do
                    local btn_name = (card.title and #card.title > 0) and card.title or ("Карточка #"..i)
                    
                    if imgui.Button(u8(btn_name) .. '##card_' .. i, imgui.ImVec2(-1, 25)) then
...
local function renderChangelogWindow()
    local sw, sh = getScreenResolution()
    local window_width = math.min(700, sw * 0.8)
    local window_height = math.min(700, sh * 0.8)
    ...
    imgui.PushStyleVarFloat(imgui.StyleVar.WindowBorderSize, 2.0)
    imgui.PushStyleColor(imgui.Col.Border, imgui.ImVec4(0.30, 0.90, 0.40, 0.90))
    imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(0.08, 0.10, 0.08, 0.98))

    local window_flags = imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove

    if main_font then imgui.PushFont(main_font) end
    if imgui.Begin(u8'Обновление установлено!##changelog', show_changelog_window, window_flags) then

        -- Заголовок
        imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.10, 0.15, 0.10, 1.00))
        imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 10.0)

        if imgui.BeginChild('##changelog_header', imgui.ImVec2(0, 100), true) then
            if title_font then imgui.PushFont(title_font) end

            local title_text = u8('ORULE обновлён до версии ' .. SCRIPT_VERSION .. '!')
            local title_width = imgui.CalcTextSize(title_text).x
            local win_width = imgui.GetWindowWidth()

            imgui.SetCursorPosY(20)
            imgui.SetCursorPosX((win_width - title_width) / 2)
            imgui.TextColored(imgui.ImVec4(0.40, 1.00, 0.50, 1.00), title_text)

            if title_font then imgui.PopFont() end

            if main_font then imgui.PushFont(main_font) end
            imgui.Spacing()
            local subtitle = u8'Спасибо за использование скрипта!'
            local subtitle_width = imgui.CalcTextSize(subtitle).x
            imgui.SetCursorPosX((win_width - subtitle_width) / 2)
            imgui.TextColored(imgui.ImVec4(0.80, 0.90, 0.80, 1.00), subtitle)
            if main_font then imgui.PopFont() end
        end
        imgui.EndChild()
        imgui.PopStyleVar(1)
        ...
        if imgui.BeginChild('##changelog_content', imgui.ImVec2(0, -65), false) then

            -- Версия 1.8
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.18, 0.12, 0.18, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v18', imgui.ImVec2(0, 178), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.90, 0.60, 1.00, 1.00), u8'  Версия 1.8 - Взаимодействие с клавишами')
                imgui.Separator()
                imgui.Spacing()
            ...
            -- Версия 1.7
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.12, 0.16, 0.12, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v17', imgui.ImVec2(0, 178), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.40, 0.80, 0.60, 1.00), u8'  Версия 1.7 - Взаимодействие с меню')
                imgui.Separator()
                imgui.Spacing()
            ...
            -- Версия 1.6
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.12, 0.18, 0.12, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v16', imgui.ImVec2(0, 285), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.40, 1.00, 0.50, 1.00), u8'  Версия 1.6 - Оптимизация и стабильность')
                imgui.Separator()
                imgui.Spacing()
                ...
                imgui.BulletText(u8'Уменьшено количество лишних вызовов resetIO()')
                imgui.Spacing()
            ...
            -- Версия 1.5
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.10, 0.12, 0.16, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v15', imgui.ImVec2(0, 178), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.50, 0.70, 1.00, 1.00), u8'  Версия 1.5 - Автовставка')
                imgui.Separator()
                imgui.Spacing()
            ...
            -- Версия 1.4
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.14, 0.10, 0.16, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v14', imgui.ImVec2(0, 155), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.85, 0.50, 1.00, 1.00), u8'  Версия 1.4 - Улучшения интерфейса')
                imgui.Separator()
                imgui.Spacing()
            ...
            -- Версия 1.3
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.16, 0.12, 0.10, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v13', imgui.ImVec2(0, 155), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(1.00, 0.70, 0.40, 1.00), u8'  Версия 1.3 - Комбинации клавиш')
                imgui.Separator()
                imgui.Spacing()
            ...
            -- Версия 1.2
            imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.10, 0.14, 0.14, 0.95))
            imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 8.0)
            if imgui.BeginChild('##v12', imgui.ImVec2(0, 178), true) then
                imgui.Spacing()
                imgui.TextColored(imgui.ImVec4(0.40, 0.90, 0.90, 1.00), u8'  Версия 1.2 - Автообновление')
                imgui.Separator()
                imgui.Spacing()
        ...
        imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.20, 0.70, 0.30, 0.90))
        imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.25, 0.80, 0.35, 1.00))
        imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.15, 0.60, 0.25, 1.00))

        local btn_width = imgui.GetMiddleButtonX(1)
        local win_width = imgui.GetWindowWidth()
        imgui.SetCursorPosX((win_width - btn_width) / 2)

        if title_font then imgui.PushFont(title_font) end
        if imgui.Button(u8'Отлично, понял!', imgui.ImVec2(btn_width, 45)) then
            show_changelog_window[0] = false
            orule.config.lastChangelogVersion = CHANGELOG_VERSION
            saveConfig()
        end
        if title_font then imgui.PopFont() end
...
local function renderInfoWindow()
    ...
    if imgui.Begin(u8'Добро пожаловать в ORULE!', show_info_window, window_flags) then
        ...
        local button_width = imgui.GetMiddleButtonX(1)
        local window_width = imgui.GetWindowWidth()
...
local function renderWindow()
    ...
    local bg_color_struct = imgui.ImVec4(0, 0, 0, 0.5)
    local bg_color_u32 = imgui.ColorConvertFloat4ToU32(bg_color_struct)
    bg_draw_list:AddRectFilled(imgui.ImVec2(0, 0), imgui.ImVec2(sw, sh), bg_color_u32)
    local adaptive_height = math.min(orule.config.windowHeight or 700, sh * 0.75)
    local adaptive_width = math.min(orule.config.windowWidth or 750, sw * 0.75)
    ...
    local titleFontApplied = false
    local mainFontApplied = false
    if title_font then
        imgui.PushFont(title_font)
        titleFontApplied = true
    end

    if imgui.Begin(u8'Менеджер правил', show_window, imgui.WindowFlags.NoCollapse) then
        if titleFontApplied then
            imgui.PopFont()
            titleFontApplied = false
        end
        if main_font then
            imgui.PushFont(main_font)
            mainFontApplied = true
        end

        if not first_render_done then
            first_render_done = true
            imgui.Text(u8'Загрузка интерфейса...')
            if mainFontApplied then imgui.PopFont() end
            imgui.End()
            return
        end
        ...
        if imgui.BeginTabBar('##main_tabs') then
            ...
            if imgui.BeginTabItem(u8'Автовставка') then
                if main_font then imgui.PushFont(main_font) end

                local content_height = imgui.GetContentRegionAvail().y
                imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.0, 0.0, 0.0, 0.0))
                if imgui.BeginChild('##autoinsert_scroll', imgui.ImVec2(-1, content_height), false) then
                    imgui.Spacing()
                    renderAutoInsertTab()
                end
                imgui.EndChild()
            ...
            if imgui.BeginTabItem(u8'Взаимодействие') then
                if main_font then imgui.PushFont(main_font) end

                local content_height = imgui.GetContentRegionAvail().y
                imgui.PushStyleColor(imgui.Col.ChildBg, imgui.ImVec4(0.0, 0.0, 0.0, 0.0))
                if imgui.BeginChild('##interaction_scroll', imgui.ImVec2(-1, content_height), false) then
                    imgui.Spacing()
    ...
    imgui.SetNextWindowPos(imgui.ImVec2(imgui.GetIO().DisplaySize.x * 0.5, imgui.GetIO().DisplaySize.y * 0.5), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))

    if should_open_radial_modal then
        imgui.OpenPopup('##radial_edit_modal')
        should_open_radial_modal = false
    end

    if imgui.BeginPopupModal('##radial_edit_modal', show_radial_edit_modal, imgui.WindowFlags.NoResize) then
        imgui.TextColored(imgui.ImVec4(0.90, 0.85, 1.00, 1.00), u8('Редактирование отыгровки'))
        imgui.Separator()
        imgui.Spacing()

        imgui.Text(u8('Название отыгровки:'))
        imgui.PushItemWidth(-20)
        imgui.InputText('##radial_name', radial_edit_name_buffer, 64)
        imgui.PopItemWidth()

        imgui.Spacing()
        imgui.Spacing()
        if radial_edit_mode == "content" then
            imgui.Text(u8('Содержимое отыгровки:'))
            imgui.InputTextMultiline('##radial_content', radial_edit_content_buffer, 4096, imgui.ImVec2(-20, 250))
        else
            imgui.Text(u8('Список доступных функций и тегов:'))
            imgui.Spacing()
            ...
            imgui.Text(u8('- '))
            imgui.SameLine()
            if imgui.Button(u8('<X>'), imgui.ImVec2(50, 30)) then
                ...
                sampAddChatMessage('[ORULE] Скопировано в буфер обмена: <X>', 0x00FF00)
            end
            imgui.SameLine()
            imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('- Задержка отыгровки, время указывается в миллисекундах. 1 секунда = 1000 миллисекунд'))
            imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8('  Например: /me ухватил{??} за дробовик\\n<2000>\\n/me нажал на курок'))

            imgui.Text(u8('- '))
            imgui.SameLine()
            if imgui.Button(u8('*'), imgui.ImVec2(30, 30)) then
                if setClipboardText then
                    setClipboardText("*")
                else
                    local user32 = ffi.load('user32')
                ...
                sampAddChatMessage('[ORULE] Скопировано в буфер обмена: *', 0x00FF00)
            end
            imgui.SameLine()
            imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('- Делает скриншот экрана'))
            imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8('  Например: /time\\n<300>\\n*'))
            ...
            imgui.SameLine()
            imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('- Вписывает в поле ввода чата указанный текст и открывает чат'))
            imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8('  Например: >>/cuff'))
            ...
            imgui.Text(u8('- '))
            imgui.SameLine()
            if imgui.Button(u8('{time}'), imgui.ImVec2(70, 30)) then
                if setClipboardText then
                ...
                sampAddChatMessage('[ORULE] Скопировано в буфер обмена: {time}', 0x00FF00)
            end
            imgui.SameLine()
            imgui.TextColored(imgui.ImVec4(0.8, 0.8, 0.8, 1.0), u8('- возвращает текущее время по МСК.'))
            imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8('  Например: /do Текущее время на часах: {time}.'))
        end

        imgui.Spacing()
        imgui.Spacing()

        local button_width = (imgui.GetContentRegionAvail().x - imgui.GetStyle().ItemSpacing.x * 2) / 3

        if imgui.Button(u8('Сохранить'), imgui.ImVec2(button_width, 35)) then
            local title_utf8 = ffi.string(radial_edit_name_buffer):gsub('\0', '')
            local content_utf8 = ffi.string(radial_edit_content_buffer):gsub('\0', '')
            local title_cp1251 = u8:decode(title_utf8)
            local content_cp1251 = u8:decode(content_utf8)

            if #title_cp1251 == 0 then
                sampAddChatMessage('[ORULE] Название не может быть пустым', 0xFF0000)
            elseif #title_cp1251 > 63 then
                sampAddChatMessage('[ORULE] Название слишком длинное', 0xFF0000)
            else
                local userData = loadUserQMs()

                while #userData < radial_edit_index do
                    table.insert(userData, {title = "", content = "", enabled = true})
                end

                userData[radial_edit_index] = {
                    title = title_cp1251,
                    content = content_cp1251,
                    enabled = radial_edit_enabled[0]
                }

                if radialMenu.buttons[radial_edit_index] then
                    radialMenu.buttons[radial_edit_index].name = title_cp1251
                    radialMenu.buttons[radial_edit_index].enabled = radial_edit_enabled[0]
                end

                if saveUserQMs(userData) then
                    show_radial_edit_modal[0] = false
                    imgui.CloseCurrentPopup()
                    sampAddChatMessage('[ORULE] Отыгровка #' .. radial_edit_index .. ' сохранена!', 0x00FF00)
                end
            end
        end

        imgui.SameLine()
        local tagsButtonText = radial_edit_mode == "content" and u8('Теги и функции') or u8('К содержимому')
        if imgui.Button(tagsButtonText, imgui.ImVec2(button_width, 35)) then
            if radial_edit_mode == "content" then
                radial_edit_mode = "tags"
            else
                radial_edit_mode = "content"
            end
        end
        ...
        imgui.SameLine()
        if imgui.Button(u8('Отмена'), imgui.ImVec2(button_width, 35)) then
            show_radial_edit_modal[0] = false
            imgui.CloseCurrentPopup()
        end
...
local function renderRadialMenu()
    ...
    for i = 1, segments do
        ...
        local textRadius = innerRadius + (outerRadius - innerRadius) * 0.35
...
local function shouldProcessImGui()
    return show_window[0] or show_info_window[0] or show_changelog_window[0] or radialMenu.active or radialMenu.pendingActivation or radialMenu.releasePending or radialMenu.isHeld
end

imgui.OnFrame(function()
    return shouldProcessImGui() or show_changelog_window[0] or show_player_menu[0] or show_interaction_editor[0] or (orule.config.autoInsertEnabled and sampIsChatInputActive())
end, function()
    imgui.Process = shouldProcessImGui() or show_changelog_window[0] or show_player_menu[0] or show_interaction_editor[0]
    if show_changelog_window[0] then
        renderChangelogWindow()
    end
    if show_info_window[0] then
        renderInfoWindow()
    end
    if show_interaction_editor[0] then
...
imgui.OnFrame(function() return show_player_menu[0] end, function()
    if main_font then imgui.PushFont(main_font) end
    ...
    if imgui.Begin(u8'Взаимодействие с игроком', show_player_menu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
        if target_ped_handle and doesCharExist(target_ped_handle) then
            local result, id = sampGetPlayerIdByCharHandle(target_ped_handle)
            if result then
                ...
                -- Загружаем отыгровки взаимодействия
                local interactionQMs = loadInteractionQMs()
                ...
                for i, item in ipairs(interactionQMs) do
                    if item.enabled then
                        if imgui.Button(u8(item.title), imgui.ImVec2(-1, 35)) then
                            ...
                            -- Заменяем теги взаимодействия
                            local content = item.content
                            content = content:gsub("{player}", player_name)
                            content = content:gsub("{id}", tostring(id))
                            content = content:gsub("{search_result}", "Ничего не найдено") -- По умолчанию

                            -- Выполняем отыгровку через существующую систему
                            local commands = parseRoleplayCommands(content)
                            if #commands > 0 then
                                executeRoleplay(commands)
                            end

                            show_player_menu[0] = false
                        end
                    end
                end
...
function onWindowMessage(msg, wparam, lparam)
    if (msg == wm.WM_KEYDOWN or msg == wm.WM_SYSKEYDOWN) then
        ...
        -- Обработка захвата клавиш для взаимодействия
        if interaction_key_capture_mode == "waiting" and interaction_key_capture_index > 0 then
            ...
            elseif code ~= 0x10 and code ~= 0x11 and code ~= 0x12 then
                -- Захватываем клавишу/комбинацию
                local shortcut_keys = {}
                ...
                if #shortcut_keys >= 1 and #shortcut_keys <= 3 then
                    -- Сохраняем бинд
                    interaction_key_binds[interaction_key_capture_index].key = shortcut_keys
                    saveInteractionKeyBinds(interaction_key_binds)
...
local function toggle()
    if orule.config.firstLaunch and not info_window_shown_once then
        ...
        imgui.Process = true
    elseif (orule.config.lastChangelogVersion or "0") ~= CHANGELOG_VERSION then
        -- Показываем changelog если версия изменилась
        show_changelog_window[0] = true
        imgui.Process = true
    else
...
local function checkMoonLoaderVersion()
    ...
    if ml_version < MINIMUM_MOONLOADER_VERSION then
...
function safeMain()
    ...
    -- Показываем changelog если версия изменилась
    if orule.config.lastChangelogVersion ~= CHANGELOG_VERSION then
        show_changelog_window[0] = true
        imgui.Process = true
    end
    ...
    -- Загружаем бинды взаимодействия
    interaction_key_binds = loadInteractionKeyBinds()
    ...
    while true do
        ...
        -- Логика взаимодействия с игроками
        if orule.config.interactionEnabled then
            if orule.config.interactionMode == 1 then
                -- Режим меню
                -- Проверяем, целится ли игрок (ПКМ) в другого персонажа
                local result, ped = getCharPlayerIsTargeting(PLAYER_HANDLE)

                -- Если цель есть, персонаж существует и нажат L.ALT + ПКМ
                if result and doesCharExist(ped) and isKeyDown(vkeys.VK_LMENU) and isKeyDown(vkeys.VK_RBUTTON) then
                    -- Сохраняем цель и открываем меню
                    target_ped_handle = ped
                    show_player_menu[0] = true
                    imgui.Process = true

                    -- Ждем отпускания клавиш, чтобы меню не мерцало
                    while isKeyDown(vkeys.VK_LMENU) or isKeyDown(vkeys.VK_RBUTTON) do
                        wait(10)
                    end
                end

                -- Закрытие меню на ESC
                if show_player_menu[0] and isKeyJustPressed(vkeys.VK_ESCAPE) then
                    show_player_menu[0] = false
            ...
            elseif orule.config.interactionMode == 0 then
                ...
                if result and doesCharExist(ped) then
                    ...
                    -- Обрабатываем бинды клавиш
                    for _, bind in ipairs(interaction_key_binds) do
                        if bind.key and #bind.key > 0 then
                            ...
                            if all_pressed then
                                -- Нашли совпадение, выполняем отыгровку
                                local interactionQMs = loadInteractionQMs()
                                for _, item in ipairs(interactionQMs) do
                                    if item.action == bind.action and item.enabled then
                                        ...
                                        -- Ждем отпускания клавиш, чтобы не повторялось
                                        while all_pressed do
                                            wait(10)
                                            all_pressed = true
                                            for _, key_code in ipairs(bind.key) do
                                                if not isKeyDown(key_code) then
        ...
        if is_game_focused and not is_capturing_keys then
            ...
            if not held_rule_active and overlay_visible then
                local rule = orule.rulesDB[overlay_rule_index]
...
function onScriptTerminate(script, quitGame)
    if script == thisScript() then
        if radar_map_texture then
            imgui.ReleaseTexture(radar_map_texture)
            radar_map_texture = nil
        end
        for i, tex in pairs(territory_textures) do
            if tex then
                imgui.ReleaseTexture(tex)
                territory_textures[i] = nil
            end
        end
    end
end
